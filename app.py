...
with tab4:
    ...
    # 5. Bar Chart: Volume Supplied by Each Blowing Plant
    st.subheader("🏭 Volume Supplied by Each Blowing Plant")

    # Merge filtered_data with Port to get blowing plant info
    ...
    plant_volume = (
        ...
        .sum()
        .reset_index()
        .sort_values(by="Volume_Million_Pieces", ascending=False)
    )

    fig_plant_volume = px.bar(
        plant_volume,
        x="blowing_plant", y="Volume_Million_Pieces", text_auto=".2s",
